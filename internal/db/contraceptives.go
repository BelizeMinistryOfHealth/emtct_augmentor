...
func (d *EmtctDb) ContraceptivesUsedByPatientId(patientId int) ([]models.ContraceptiveUsed, error) {
	stmt := `
		SELECT id, patient_id, contraceptive, comments, date_used, created_at, created_by, 
       		updated_at, updated_by, mch_encounter_id 
		FROM contraceptive_used WHERE patient_id=$1`
...
func (d *EmtctDb) CreateContraceptiveUsed(c models.ContraceptiveUsed) error {
	...
	_, err := d.DB.Exec(stmt, c.Id, c.PatientId, c.Contraceptive, c.Comments, c.DateUsed, c.CreatedBy, c.CreatedAt, c.MchEncounterId)
	if err != nil {
...
func (d *EmtctDb) FindContraceptiveById(id string) (*models.ContraceptiveUsed, error) {
	stmt := `
		SELECT id, patient_id, contraceptive, comments, created_at, created_by, updated_at, 
			updated_by, mch_encounter_id 
		FROM contraceptive_used WHERE id=$1`

...
func (a *App) CreateHivScreeningHandler(w http.ResponseWriter, r *http.Request) {
	...
	infant, err := a.AcsisDb.FindInfant(req.PatientId)
	if err != nil {
	...
	timely := models.IsHivScreeningTimely(*infant.Infant.Dob, req.TestName, req.DateSampleTaken)
	dueDate := models.HivScreeningDueDate(req.TestName, *infant.Infant.Dob)
...
func (a *App) HivScreeningsByPatientIdHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		infant, err := a.AcsisDb.FindInfant(id)
		if err != nil {
	...
	case http.MethodPost:
		a.CreateHivScreeningHandler(w, r)
	}
}
...
func (a *App) HivScreeningApi(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPut:
		...
		if err != nil {
			...
			http.Error(w, fmt.Sprintf("no birth was found for infant Id: %s", req.PatientId), http.StatusBadRequest)
			return
		}
		timely := models.IsHivScreeningTimely(*infant.Infant.Dob, req.TestName, *req.DateSampleTaken)
		req.UpdatedBy = &user
